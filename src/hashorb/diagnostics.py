...
from hashorb.compute import LocalComputeProfileCapabilities, PythonSequentialBackend
...
def _version_check() -> DoctorCheck:
    try:
        value = metadata.version("hashorb")
    except metadata.PackageNotFoundError:
        return DoctorCheck("hashorb-version", DoctorStatus.ERROR, "package metadata unavailable")
    if not value or value != value.strip():
        return DoctorCheck("hashorb-version", DoctorStatus.ERROR, "package metadata invalid")
    return DoctorCheck("hashorb-version", DoctorStatus.READY, value)
...
def _installation_type() -> str:
    try:
        distribution = metadata.distribution("hashorb")
    except metadata.PackageNotFoundError:
...
def _cuda_extension_check() -> DoctorCheck:
    try:
        available = find_spec("hashorb.compute._cuda") is not None
...
def _configuration_checks(
    ...
    *,
    ...
    address_present = bool(environment.get("HASHORB_BITCOIN_ADDRESS", "").strip())
    return (
        DoctorCheck(
