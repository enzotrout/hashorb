...
class BitcoinRpcSettings:
    ...
    def from_env(cls) -> BitcoinRpcSettings:
        ...
        host = os.getenv("HASHORB_BITCOIN_RPC_HOST", DEFAULT_BITCOIN_RPC_HOST)
        port = _parse_environment_integer(
            os.getenv("HASHORB_BITCOIN_RPC_PORT", str(DEFAULT_BITCOIN_RPC_PORT)),
        ...
        username = _optional_environment_text("HASHORB_BITCOIN_RPC_USER")
        password = _optional_environment_text("HASHORB_BITCOIN_RPC_PASSWORD")
        cookie_text = _optional_environment_text("HASHORB_BITCOIN_RPC_COOKIE_FILE")
        return cls(
            host=host,
            port=port,
...
def _parse_timeout(value: object) -> float:
    if not isinstance(value, str) or not value or value != value.strip():
        raise ValueError("HASHORB_BITCOIN_RPC_TIMEOUT_SECONDS must be a finite decimal")
    try:
        parsed = float(value)
    except ValueError as exc:
        raise ValueError("HASHORB_BITCOIN_RPC_TIMEOUT_SECONDS must be a finite decimal") from exc
    if not math.isfinite(parsed) or not 0 < parsed <= MAX_BITCOIN_RPC_TIMEOUT_SECONDS:
        raise ValueError("HASHORB_BITCOIN_RPC_TIMEOUT_SECONDS must be positive and at most 120")
    return parsed
