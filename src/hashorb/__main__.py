"""Command-line entry point for HashOrb development operations."""
...
from hashorb.bitcoin.command import BITCOIN_COMMANDS, run_bitcoin_command
...
_LIVE_STRATUM_FLAG = "HASHORB_ENABLE_LIVE_STRATUM"
_LIVE_MINING_FLAG = "HASHORB_ENABLE_LIVE_MINING"
_STRATUM_USER_AGENT = "HashOrb/0.1"
_NONCE_LIMIT = 1 << 32
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if arguments and arguments[0] == "compute-benchmark":
        ...
        profile_requested = "--profile" in arguments[1:] or (
            "--backend" not in arguments[1:] and os.getenv("HASHORB_COMPUTE_PROFILE") is not None
        )
...
def _print_compute_benchmark(
    ...
    print("HashOrb compute benchmark completed.")
...
def _print_compute_benchmark_series(
    ...
    capabilities = backend.capabilities
    print("HashOrb repeated compute benchmark completed.")
    print(f"Backend: {capabilities.backend_name}")
    print(f"Implementation: {capabilities.implementation}")
    worker_count = compute_backend_worker_count(backend)
    if worker_count is not None:
...
def _run_doctor(
    ...
    load_hashorb_environment()
    profile_requested = (
        selection.profile_name is not None or os.getenv("HASHORB_COMPUTE_PROFILE") is not None
    )
    profile_error = False
...
def _resolve_command_profile(
    ...
    *,
    ...
    load_hashorb_environment()
    environment_profile = (
        os.getenv("HASHORB_COMPUTE_PROFILE") if selection.use_environment_profile else None
    )
    ...
    if requested is None:
        if require_profile:
            raise ValueError("--profile or HASHORB_COMPUTE_PROFILE is required")
...
def _print_resolved_compute_profile(profile: ResolvedComputeProfile) -> None:
    ...
    print("HashOrb compute profile resolved.")
    print(f"Requested profile: {profile.requested_profile}")
    print(f"Effective profile: {profile.effective_profile}")
...
def _parse_profiled_continuous_mining_arguments(
    ...
    has_environment_profile = (
        "--chunk-size" not in option_values and os.getenv("HASHORB_COMPUTE_PROFILE") is not None
    )
