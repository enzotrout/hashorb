...
from hashorb.config import DEFAULT_CUDA_DEVICE, MAX_CUDA_DEVICE
...
_CUDA_EVALUATED_THREADS_PER_BLOCK = frozenset({64, 128, 256, 512})


def _cuda_launch_geometry(
    range_size: int,
    ...
    if parsed_threads not in _CUDA_EVALUATED_THREADS_PER_BLOCK:
    ...
    return min(required_blocks, _CUDA_MAX_BLOCKS), parsed_threads
...
class CudaBackend:
    ...
    def _initialize_runtime(self, runtime: object) -> str | None:
        ...
        if selected_runtime is _AUTO_LOAD:
            try:
                selected_runtime = import_module("hashorb.compute._cuda")
            except ImportError:
