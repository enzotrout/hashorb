...
from hashorb.bitcoin.template import BlockTemplate, BlockTemplateError, parse_block_template
...
from hashorb.observability import EventLogError, EventSink, JsonlEventSink, NullEventSink
...
def _command_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="hashorb", exit_on_error=False)
...
def _resolve_solo_policy(
    ...
    profile_name = parse_compute_profile(
        arguments.profile or os.getenv("HASHORB_COMPUTE_PROFILE", "auto")
    )
    ...
    strategy = select_search_strategy(
        arguments.strategy or os.getenv("HASHORB_SEARCH_STRATEGY", "sequential")
    )
    plan = SoloMiningPlan(
