...
**What:** HashOrb has one CPython 3.13 package, an installed `hashorb`
...
not set `HASHORB_BUILD_CUDA=1`. They do not invoke `nvcc`, link a CUDA runtime,
...
unprivileged `hashorb` account with numeric UID/GID 10001. `/app/logs` is
writable and declared as a volume. The offline doctor is also the health check.

The JSON exec entry point is `hashorb`, so there is no shell or uv parent
...
```bash
hashorb doctor --probe-cuda-device 0
```
...
./scripts/install-unix.sh install
hashorb doctor
hashorb profile-info --profile auto
...
HashOrb uses `pathlib`, explicit UTF-8 JSON/JSONL I/O, and the `.exe` console
...
HashOrb discovers `.env` from the current working directory through
...
`--log-file '.\HashOrb Logs\events.jsonl'`.
...
docker run --rm -v hashorb-logs:/app/logs hashorb:cpu \
...
Python 3.13 range, README metadata, and `hashorb` console entry point. No OS
...
claims remain blocked until that legal choice is made. HashOrb does not
invent a license.
...
`Scripts/hashorb.exe` on Windows versus `bin/hashorb` on POSIX; no shell
...
The package under `src/hashorb` remains the only miner. Platform directories
